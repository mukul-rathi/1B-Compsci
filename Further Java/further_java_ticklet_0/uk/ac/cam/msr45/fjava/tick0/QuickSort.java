...
public class QuickSort{
    private static boolean mRandomised = true; //true => randomised pivot, false => pivot = left-most element



    private static int partition(int[] A, int p, int q){ //note we are considering A[p...q] inclusive
    ...
    private static void sortHelper(int[] A, int p, int q){ //note we are considering A[p...q] inclusive
        //if p==q then base case: size of array = 1 so sorted
        if(p<q && !isSorted(A, p, q)){ //i.e size of Array>1
    ...
    private static boolean isSorted(int[] a, int p, int q) {
        boolean sort = true;
        for(int i=p; i<q; i++){
            if(a[i+1]<a[i]){
                sort=false;
